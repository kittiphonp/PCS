...
//Hybrid Version for saving additions
//Montgomery + Edwards
void yDBL(const point_proj_t P, point_proj_t Q, const f2elm_t A24plus, const f2elm_t C24)
{   // Based on https://eprint.iacr.org/2019/837
    // Input: projective Edwards point P = (X1 - Z1:X1 + Z1),  and Montgomery curve constants A+2C and 4C.
    // Output: projective Edwards Edwards point Q = 2*P = (X2 - Z2 :X2 + Z2).
    f2elm_t t0, t1;
    
    fp2sqr_mont(P->X, t0);                            // t0 = (X1-Z1)^2 
    fp2sqr_mont(P->Z, t1);                            // t1 = (X1+Z1)^2 
    fp2mul_mont(C24, t0, Q->Z);                     // Z2 = C24*(X1-Z1)^2   
    fp2mul_mont(t1, Q->Z, Q->X);                    // X2 = C24*(X1-Z1)^2*(X1+Z1)^2
    fp2sub(t1, t0, t1);                             // t1 = (X1+Z1)^2-(X1-Z1)^2 
    fp2mul_mont(A24plus, t1, t0);                   // t0 = A24plus*[(X1+Z1)^2-(X1-Z1)^2]
    fp2add(Q->Z, t0, Q->Z);                         // Z2 = A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2
    fp2mul_mont(Q->Z, t1, t0);                    // Z2 = [A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2]*[(X1+Z1)^2-(X1-Z1)^2]
    
    fp2add(Q->X, t0, Q->Z);                         // Z2 = X2+Z2
    fp2sub(Q->X, t0, Q->X);                         // X2 = X2-Z2
}
// 
...
void eval_4_isog_mod(point_proj_t Q, point_proj_t P, f2elm_t* coeff)
{ // Evaluates the isogeny at the point (X:Z) in the domain of the isogeny, given a 4-isogeny phi defined 
  // by the 3 coefficients in coeff (computed in the function get_4_isog()).
  // Inputs: the coefficients defining the isogeny, and the projective point P = (X:Z).
  // Output: the projective point P = phi(P) = (X:Z) in the codomain. 
...
void eval_3_isog_mod(point_proj_t Q, point_proj_t P, const f2elm_t* coeff)
{ // Computes the 3-isogeny R=phi(X:Z), given projective point (X3:Z3) of order 3 on a Montgomery curve and 
  // a point P with 2 coefficients in coeff (computed in the function get_3_isog()).
  // Inputs: projective points P = (X3:Z3) and Q = (X:Z).
  // Output: the projective point Q <- phi(Q) = (X3:Z3). 
    ...
    fp2sqr_mont(t2, t2);                          // t2 = [coeff0*(X+Z) + coeff1*(X-Z)]^2
    fp2sqr_mont(t0, t0);                          // t0 = [coeff1*(X-Z) - coeff0*(X+Z)]^2
    fp2mul_mont(Q->X, t2, P->X);                  // X3final = X*[coeff0*(X+Z) + coeff1*(X-Z)]^2        
    fp2mul_mont(Q->Z, t0, P->Z);                  // Z3final = Z*[coeff1*(X-Z) - coeff0*(X+Z)]^2
}

//New functions for [5], get 5-isogeny and eval 5-isogeny
...
{       
        ...
        // Input: projective Montgomery x-coordinates P = (X:Z), where x=X/Z and Montgomery curve constants A24plus = A+2C , A24minus = A-2C and C24 = 4C.
        // Output: projective Montgomery x-coordinates Q = 5*P = (X3:Z3).
        
        //Based on "A simple and compact algorithm for SIDH with arbitrary degree isogenies" by Costello and Hisil.
        //https://eprint.iacr.org/2017/504

        f2elm_t r1, r2, r3, r4, r5;

        fp2sub(P->X, P->Z, r1);                               // R1 := X - Z;//a
        fp2sqr_mont(r1, r2);                                  // R2 := R1^2;//S
        fp2add(P->X, P->Z, r3);                               // R3 := X + Z;//a
        fp2sqr_mont(r3, r4);                                  // R4 := R3^2;//S
        fp2add(r4, r2, r5);                                   // R5 := R4 + R2;//a
        fp2add(r1, r3, r1);                                   // R1 := R1 + R3 ;//a
        fp2sqr_mont(r1, r1);                                  // R1 := R1^2;//S
        fp2sub(r1, r5, r3);                                   // R3 := R1 - R5 ;//a
        fp2mul_mont(A24m, r2, r2);                            // R2 := R2 * A24m ;//M
        fp2mul_mont(r4, A24p, r4);                            // R4 := A24p * R4 ;//M
        fp2sub(r4, r2, r4);                                   // R4 := R4 - R2;//a
        fp2mul_mont(r4, r5, r5);                              // R5 := R4 * R5 ;//M
        fp2sqr_mont(r3, r2);                                  // R2 := R3^2;//S
        fp2mul_mont(r2, C24, r1);                             // R1 := R2 * C24;//M
        fp2div2(r1,r2);                                       // R2 := R1 / 4;//div 4
        fp2div2(r2, r2);    
        fp2sub(r5, r2, r2);                                    // R2 := R5 - R2 ;//a
        fp2mul_mont(r4, r3, r5);                               // R5 := R4 * R3 ;//M
        fp2add(r2, r5, r3);                                    // R3 := R2 + R5;//a
        fp2sub(r2, r5, r4);                                    // R4 := R2 - R5;//a
        fp2mul_mont(r3, r4, r3);                               // R3 := R3 * R4;//M 
        fp2mul_mont(r2, r3, r4);                               // R4 := R2 * R3;//M
        fp2mul_mont(r1, r2, r1);                               // R1 := R1 * R2;//M
        fp2add(r1, r3, r1);                                    // R1 := R1 + R3 ;//a
        fp2mul_mont(r1, r5, r3);                               // R3 := R1 * R5 ;//M
        fp2add(r4, r3, r2);                                    // R2 := R4 + R3;//a
        fp2sqr_mont(r2, r2);                                   // R2 := R2^2;//S
        fp2mul_mont(r2, P->X, Q->X);                           // Xout := R2 * X ;//M
        fp2sub(r4, r3, r1);                                    // R1 := R4 - R3 ;//a
        fp2sqr_mont(r1, r1);                                   // R1 := R1^2;//S
        fp2mul_mont(r1, P->Z, Q->Z);                           // Zout := R1 * Z;//M
} //11M + 6S + 15A
...
void xQPLe(const point_proj_t P, point_proj_t Q, const f2elm_t A24p, const f2elm_t A24m,  const f2elm_t C24, int e)  
{
        ...
        copy_words((digit_t*)P, (digit_t*)Q, 2*2*NWORDS_FIELD);
        for (i = 0; i < e; i++) {
                xQPL(Q, Q, A24p, A24m, C24);
        }
}

void eval_5_isog(const point_proj_t *KPs, point_proj_t Q)
{
    //Function based on https://eprint.iacr.org/2019/837
    //Input: The set KPS which holds the Points in
    //          Edwards coordintaes
    //          {(X1 - Z1 : X1 + Z1), (X2-Z2: X2 + Z2)}
    //          Where (X1: Z1) is an order five point in Montgomery coordinates,
    //          And a Point Q in Montgomery Coordinates to be evaluated
    //Return: Point Q evaluated trough a 5-degree isogeny.
    
    f2elm_t t00, t01, t3, t4, X0, Z0, X1, Z1;
    
    fp2sub(Q->X, Q->Z, t00);//         t00 := (X - Z);              //a
    fp2add(Q->X, Q->Z, t01);//         t01:= (X + Z);               //a
    fp2mul_mont(t00, KPs[0]->Z, t3);//         t3 := t00 * t1;                                        //M
    fp2mul_mont(t01, KPs[0]->X, t4);//         t4 := t01*t1;                                         //M
    fp2add(t3, t4, X0);//         X0 := t3 + t4;                                        //a
    fp2sub(t3, t4, Z0);//         Z0 := t3 - t4;                                        //a
    fp2mul_mont(t00, KPs[1]->Z, t3);//         t3 := t00 * t1;                                                //M
    fp2mul_mont(t01, KPs[1]->X, t4);//         t4 := t01*t1;                                                //M
    fp2add(t3, t4, X1);//         X1 := t3 + t4;                                              //a
    fp2sub(t3, t4, Z1);//         Z1 := t3 - t4;                                               //a
    fp2mul_mont(X0, X1, t3);//         Xn := X0 * X1;                                //M
    fp2sqr_mont(t3, t3);//         Xn := Xn^2;                                    //S
    fp2mul_mont(t3, Q->X, Q->X);//         Xn := Xn * X;                                  //M
    fp2mul_mont(Z0, Z1, t3);//         Zn := Z0 * Z1;                               //M
    fp2sqr_mont(t3, t3);//         Zn := Zn^2;                                   //S
    fp2mul_mont(t3, Q->Z, Q->Z);//         Zn := Zn * Z;                                //M
}//8M + 2S + 6A

void get_5_isog(const point_proj_t P,  point_proj_t *KPs,  f2elm_t A24plus, f2elm_t A24minus, f2elm_t C24)
{
    //Hybrid Version
    //Based on "A note on the cost of computing odd degree isogenies"
    //https://eprint.iacr.org/2019/1373
    //Input: a point P in Montgomery Coordintaes, and the Elementes A24plus, a24minus and C24 which define a
    //Montgomery curve
    //Return: New A24plus, A24minus, C24 defining a 5-isogenous curve
    //          Set KPS holding the Twisted Edwards points {(X1 - Z1 : X1 + Z1), (X2-Z2: X2 + Z2)}
    f2elm_t pY, pZ;
    //Sending The point P to Edwards representation
    fp2sub(P->X, P->Z, KPs[0]->X);    //     piY := Kx - Kz;             //A
    fp2add(P->X, P->Z, KPs[0]->Z);    //     piZ := Kx + Kz;             //A
    //Computing the yDBL as in https://eprint.iacr.org/2019/837
    yDBL(KPs[0], KPs[1], A24plus, C24);    //     Kax , Kaz := xDBL(Kx, Kz, A24p, C24);    //4M + 2S + 4A
        //     
    fp2mul_mont(KPs[0]->X, KPs[1]->X, pY);        //     piY := piY * t0;            //M
    fp2mul_mont(KPs[0]->Z, KPs[1]->Z, pZ);    //     piZ := piZ * t1;            //M
        //   
    fp2sqr_mont(pY, pY);    //     piY := piY^2;               //S
    fp2sqr_mont(pZ, pZ);    //     piZ := piZ^2;               //S
    fp2mul_mont(pY, A24minus, pY);    //     piY := piY * A24m;          //M
    fp2mul_mont(pZ, A24plus, pZ);    //     piZ := piZ * A24p;          //M
    fp2sqr_mont(pY, pY);    //     piY := piY^2;               //S
    fp2sqr_mont(pY, pY);    //     piY := piY^2;               //S
    fp2sqr_mont(pZ, pZ);    //     piZ := piZ^2;               //S
    fp2sqr_mont(pZ, pZ);    //     piZ := piZ^2;               //S
    fp2mul_mont(pY, A24minus, A24minus);    //     piY := piY * A24m;          //M
    fp2mul_mont(pZ, A24plus, A24plus);    //     piZ := piZ * A24p;          //M
    fp2sub(A24plus, A24minus,C24);          //A
} //Cost 10M + 8S + 9A
